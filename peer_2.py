...
class Peer:
    def __init__(self):
        ...
        self.bitfield = ["1", "1", "0", "0", "1", "0"]
        self.filename = ["piece_1.txt", "piece_2.txt", "", "", "piece_3.txt", ""]
                    ...
                    if raw_info.startswith("/download"):
                        ...
                        for info in infos:
                            if info.startswith("piece="):
                                ...
                                filepath = os.path.join(self.directory, self.filename[piece_num])
                                with open(filepath, "r") as infile:
                                    ...
                                    print(f"[{self.address}] Sending response to {source_address}")
    ...
    async def connect_peer(self, ip, port, piece):
        ...
        print(f"[{self.address}] Connected to {destination_address} and ready to make a request")
        request = (
        ...
        response = await reader.read(1500)
        print(f"[{self.address}] Received a response from {destination_address}")
        res = response.decode()
        data = res.split('\r\n')[-1]
        if data:
    ...
    def check_and_combine(self):
        if self.bitfield[0] == "1" and self.bitfield[1] == "1" and self.bitfield[2] == "1":
            filename = "file_1.txt"
            filepath = os.path.join(self.directory, filename)
            print(f"[{self.address}] Combining pieces of file 1")
            with open(filepath, "w") as outfile:
                for index in range(3):
                    infilepath = os.path.join(self.directory, self.filename[index])
        ...
        if self.bitfield[3] == "1" and self.bitfield[4] == "1" and self.bitfield[5] == "1":
            filename = "file_2.txt"
            filepath = os.path.join(self.directory, filename)
...
if __name__ == "__main__":
    peer1 = Peer()
    # peer_list = asyncio.run(peer1.connect_tracker())
